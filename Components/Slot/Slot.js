...
const Slot = ({ index, filled, handleOnPress }) => {
  ...
  const onTap = event => {
    if (event.nativeEvent.state === State.ACTIVE) {
      Alert.alert(
        'Hint',
        'You need to press longer or double tab the slot to make your move!',
      );
    }
  };
  ...
  return (
    <LongPressGestureHandler onHandlerStateChange={onDoubleTap} minDurationMs={500}>
      <TapGestureHandler onHandlerStateChange={onTap} waitFor={doubleTapRef}>
        <TapGestureHandler ref={doubleTapRef} shouldCancelWhenOutside={true} onHandlerStateChange={onDoubleTap} numberOfTaps={2}>
          <SlotWrapper>
